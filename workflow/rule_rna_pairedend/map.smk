rule bowtie2_index:
    ...
    conda:
        "conda_env/rule_rna_pairedend/map"
    log:
...
rule bowtie2_map:
    ...
    output:
        ...
        # unpaired = "results/Mapping/{species}/{sample}/bowtie2/unpaired_{sample}.fastq.gz"  ###            --un-gz {output.unpaired} \
    threads: 8
    params:
        prefix = "data/reference/{species}/{species}"
    conda:
...
rule samtools_view:
    ...
    output:
        bam = "results/Mapping/{species}/{sample}/samtools/bam_{sample}.bam"
    threads: 8
    params:
        prefix = "data/reference/{species}/{species}"
    conda:
